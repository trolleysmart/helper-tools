...
const start = async () => {
  ...
  const splittedCrawledStoreProducts = ImmutableEx.splitIntoChunks(crawledStoreProducts, 100);

  await BluebirdPromise.each(splittedCrawledStoreProducts.toArray(), async (crawledStoreProductsChunck) => {
    const crawledProductPricesPromises = crawledStoreProductsChunck.map(crawledStoreProduct =>
      loadLatestCrawledProductPrice(storeId, crawledStoreProduct.get('id')));
    const crawledProductPrices = await Promise.all(crawledProductPricesPromises.toArray());

    crawledStoreProductsChunck.forEach((crawledStoreProduct) => {
